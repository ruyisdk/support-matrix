...
# RT-Thread HengShanPi Test Report

## Test Environment

### Hardware Information

- Development Board: HengShanPi D133EBS
- Other Hardware:
...
- OS Version: RT-Thread v4.1.1 (ArtInChip Luban-lite SDK)
...
```bash
#scons --list-def //Check how many configurations there are
#scons --apply-def=0 //Select No.0 configure
...
It can be further configured using `menuconfig` on the basis of the default configuration
...
The compiled image `d13x_JLC_v1.0.0.img` will be saved in the directory `output/d13x_JLC_rt-thread_helloworld/images`
...
Using ArtInChip's [`AiBurn`](https://gitee.com/artinchip/tools/blob/master/AiBurn-1.4.7_Setup.zip) softwave flash the image directly via USB (Windows environment only), or usb MicroSD card to flash the image.

> When flashing image with a MicroSD card, it is necessary to ensure that the `bootloader.aic` file generated by compilation is less than 126 KB. Specific operation see the [docs](https://wiki.lckfb.com/zh-hans/hspi-d133ebs/rtos-sdk/user-guide/burn-image.htm).

### Logging into the System

Connect to the development board via serial port (Baudrate: 115200).
